...
local routes = {
    {name = "Cursed Tank Simulator", placeid = 6608498361, gameid = 2491559356, subplaces = true,
        profileFolder = "hitechy/cursed-tank/profiles", settingsPrefix = "BuiltInSettings",
        url = "https://luaprot.net/api/v3/loaders/get/24826552930370898475"},
    {name = "CENTAURA", placeid = 8735521924,
        profileFolder = "hitechy/centaura/profiles", settingsPrefix = "CentauraSettings",
        url = "https://luaprot.net/api/v3/loaders/get/08694063717521711286"},
    {name = "Cold War", placeid = 13687899540,
        profileFolder = "hitechy/cold-war/profiles", settingsPrefix = "ColdWarSettings",
        url = "https://luaprot.net/api/v3/loaders/get/66347400103882925482"},
    {name = "Wings of Glory", placeid = 338574920, gameid = 125223108, subplaces = true,
        profileFolder = "hitechy/wings-of-glory/profiles", settingsPrefix = "BuiltInSettings",
        url = "https://luaprot.net/api/v3/loaders/get/88713719087391735137"},
}




local function detect(placeid, gameid)
    for _, route in ipairs(routes) do
        if placeid == route.placeid then return route, "main place" end
    end
    for _, route in ipairs(routes) do
        if route.subplaces and gameid == route.gameid then return route, "game place" end
    end
end
...
local function applyAutoloadTheme(library, selectedRoute)
    if not (readfile and isfile) then return end

    local folders = {}
    if selectedRoute.profileFolder then table.insert(folders, selectedRoute.profileFolder) end
    table.insert(folders, "hitechy/" .. tostring(game.PlaceId) .. "/profiles")
    if game.PlaceId ~= selectedRoute.placeid then
        table.insert(folders, "hitechy/" .. tostring(selectedRoute.placeid) .. "/profiles")
    end
    local folderCount = #folders
    for index = 1, folderCount do
        local legacyFolder = folders[index]:gsub("^hitechy/", "Potas/")
        table.insert(folders, legacyFolder)
    end

    local profile
    for _, folder in ipairs(folders) do
        local autoloadPath = folder .. "/autoload.txt"
        local found, exists = pcall(isfile, autoloadPath)
        if found and exists then
            local readName, name = pcall(readfile, autoloadPath)
            name = readName and tostring(name):match("^%s*(.-)%s*$") or nil
            if name and name:match("^[%w_%-]+$") then
                local profilePath = folder .. "/" .. name .. ".json"
                local foundProfile, hasProfile = pcall(isfile, profilePath)
                if foundProfile and hasProfile then
                    local readProfile, json = pcall(readfile, profilePath)
                    if readProfile then
                        local decoded, data = pcall(function()
                            return game:GetService("HttpService"):JSONDecode(json)
                        end)
                        if decoded and type(data) == "table" then profile = data; break end
                    end
                end
            end
        end
    ...
    local function unpackColor(packed)
        if type(packed) ~= "table" or packed.__type ~= "Color3" then return nil end
        local r, g, b = tonumber(packed.r), tonumber(packed.g), tonumber(packed.b)
        if not (r and g and b) then return nil end
        return Color3.new(math.clamp(r, 0, 1), math.clamp(g, 0, 1), math.clamp(b, 0, 1))
    end

    if type(profile.theme) == "string" then library:SetTheme(profile.theme, true) end
    if type(profile.gradient) == "table" then
        library:SetGradientColors(unpackColor(profile.gradient.start), unpackColor(profile.gradient.finish))
    end
    local options = type(profile.options) == "table" and profile.options or {}
    local prefix = selectedRoute.settingsPrefix or "BuiltInSettings"
    for _, name in ipairs({"BackgroundColor", "MainColor", "AccentColor", "OutlineColor", "FontColor"}) do
        local color = unpackColor(options[prefix .. name])
        if color then library.Scheme[name] = color end
    end
...
local CreateUIInstance = Instance.new
...
local function WaitForEvent(Event, Timeout, Condition)
    local Bindable = CreateUIInstance("BindableEvent")
...
function Library:UpdateDependencyBoxes()
    if Library.ProfileLoading then
        Library.DependenciesDirty = true
        return
    end
    Library.DependenciesDirty = false
...
local function New(ClassName: string, Properties: { [string]: any }): any
    local Object = CreateUIInstance(ClassName)
    ...
    if Object:IsA("TextLabel") or Object:IsA("TextButton") or Object:IsA("TextBox") then
        local Entry = {Size = Object.TextSize}
        if not Object.TextScaled and not Object.TextWrapped and Object.AutomaticSize == Enum.AutomaticSize.None then
            local Constraint = CreateUIInstance("UITextSizeConstraint")
            Constraint.Parent = Object
            Entry.Constraint = Constraint
            Object.TextScaled = true
        end
        FontLabels[Object] = Entry
        ApplyFontSize(Object, Entry)
    end
...
function Library:SetGradientColors(Start, Finish, Animate)
    ...
    if not Animate or Library.ProfileLoading then
        if not Library.AccentGradientRefreshPending then
            Library.AccentGradientRefreshPending = true
            task.spawn(function()
                RunService.Heartbeat:Wait()
                Library.AccentGradientRefreshPending = false
                if not Library.Unloaded then
                    ApplySequence(GetGlassSequence(Library.GradientStartColor, Library.GradientEndColor))
                end
            end)
        end
        return
...
function Library:SetSchemeColor(Name, Value)
    ...
    if Library.Scheme[Name] == Value then return true end

    Library.Scheme[Name] = Value
    if Name == "BackgroundColor" then
        Library.IsLightTheme = Value.R * 0.299 + Value.G * 0.587 + Value.B * 0.114 > 0.55
    end
    Library.PendingSchemeColors = Library.PendingSchemeColors or {}
    Library.PendingSchemeColors[Name] = true
    if Library.SchemeRefreshPending then return true end


    Library.SchemeRefreshPending = true
    task.spawn(function()
        while not Library.Unloaded do
            RunService.Heartbeat:Wait()
            local Changed = Library.PendingSchemeColors
            Library.PendingSchemeColors = {}
            for Instance, Properties in pairs(Library.Registry) do
                if Instance.Parent then
                    for Property, Index in pairs(Properties) do
                        if Changed[Index] or typeof(Index) == "function" then
                            ...
                            if Target ~= nil and Instance[Property] ~= Target then
            ...
            if Changed.BackgroundColor or Changed.MainColor then
                local Sequence = ColorSequence.new(Library.Scheme.BackgroundColor, Library.Scheme.MainColor)
                for _, Gradient in ipairs(Library.DarkGradients) do
                    if Gradient.Parent then Gradient.Color = Sequence end
                end
            end
            if not next(Library.PendingSchemeColors) then break end
        end
        Library.SchemeRefreshPending = false
    end)
    return true
end
...
function Library:SetTheme(Name, Instant)
    ...
    if Instant or Library.ProfileLoading then
        Library.DisplayScheme = nil
        local Sequence = ColorSequence.new(Library.Scheme.BackgroundColor, Library.Scheme.MainColor)
        for _, Gradient in ipairs(Library.DarkGradients) do
            if Gradient.Parent then Gradient.Color = Sequence end
        end
    ...
    Library:SetGradientColors(Theme.GradientStart, Theme.GradientEnd, not Instant)
    local ThemeYieldAt = os.clock()
    for _, Toggle in pairs(Toggles) do
        if Toggle.UpdateColors then Toggle:UpdateColors() end
        if (Instant or Library.ProfileLoading) and os.clock() - ThemeYieldAt >= 0.004 then
            task.wait()
            ThemeYieldAt = os.clock()
        end
    end
    ...
    for Instance, Properties in table.clone(Library.Registry) do
        ...
        for Property, Index in Properties do
            ...
            if not Instant and not Library.ProfileLoading and typeof(Target) == "Color3" and typeof(Instance[Property]) == "Color3" then
...
function Library:SetMainMenuGradient(Info)
    ...
    if Library.MainMenuGradientMode == "Custom" then
        ...
        Overlay.BackgroundTransparency = Library.MainMenuGradientTransparency
    else
        Overlay.BackgroundTransparency = 1
        Overlay.Visible = false
    end
    Gradient.Color = ColorSequence.new(Library.MainMenuGradientStart, Library.MainMenuGradientEnd)
end
...
function Library:GetTextBounds(Text: string, Font: Font, Size: number, Width: number?): (number, number)
    local Params = CreateUIInstance("GetTextBoundsParams")
    Params.Text = Text
...
do
    ...
    function Funcs:AddViewport(Idx, Info)
        ...
        local Viewport = {
            ...
            Camera = if not Info.Camera then CreateUIInstance("Camera") else Info.Camera,
...
function Library:ExportProfile()
    ...
    for Name, Option in pairs(table.clone(Options)) do
        if Option.SkipProfile then continue end
...
function Library:ImportProfile(Json)
    ...
    Library.ProfileImportToken = (Library.ProfileImportToken or 0) + 1
    local Token = Library.ProfileImportToken
    Library.ProfileLoading = true
    local YieldAt = os.clock()
    local function YieldForUI()
        if os.clock() - YieldAt >= 0.004 then
            task.wait()
            YieldAt = os.clock()
        end
        return Library.Unloaded or Library.ProfileImportToken ~= Token
    end

    if Data.theme then Library:SetTheme(Data.theme, true) end
    if YieldForUI() then return false, "profile load interrupted" end
    ...
    local Prefix = Library.SettingsPrefix
    if Prefix and Data.theme then
        local ThemeOption = Options[Prefix .. "Theme"]
        if ThemeOption then ThemeOption:SetValue(Data.theme, nil, true) end
        for _, Name in ipairs({"GradientStart", "GradientEnd", "BackgroundColor", "MainColor", "AccentColor", "OutlineColor", "FontColor"}) do
            local Option = Options[Prefix .. Name]
            if Option then
                local Value = Name == "GradientStart" and Library.GradientStartColor
                    or Name == "GradientEnd" and Library.GradientEndColor or Library.Scheme[Name]
    ...
    local ToggleNames = {}
    for Name in pairs(Data.toggles or {}) do table.insert(ToggleNames, Name) end
    table.sort(ToggleNames)
    for _, Name in ipairs(ToggleNames) do
        if Toggles[Name] then pcall(Toggles[Name].SetValue, Toggles[Name], Data.toggles[Name]) end
        if YieldForUI() then return false, "profile load interrupted" end
    end

    local OptionNames = {}
    for Name in pairs(Data.options or {}) do table.insert(OptionNames, Name) end
    table.sort(OptionNames)
    for _, Name in ipairs(OptionNames) do
        local Option = Options[Name]
        if Option and not Option.SkipProfile and Option.SetValue then
            if Data.theme and Option.Text == "Menu Theme" then
                Option:SetValue(Data.theme, nil, true)
            else
                pcall(Option.SetValue, Option, UnpackProfileValue(Data.options[Name]))
            end
        end
        if YieldForUI() then return false, "profile load interrupted" end
    end
    Library.ProfileLoading = false
    if Library.DependenciesDirty then Library:UpdateDependencyBoxes() end
    return true, Data
end


function Library:AddCloudProfileControls(Container, Prefix)
    local Store = Library.CloudProfiles
    local Busy = false
    local Status = Container:AddLabel("Refresh to list your cloud profiles.")
    local List = Container:AddDropdown(Prefix .. "List", {Text = "Cloud profile", Values = {}, AllowNull = true})
    local Name = Container:AddInput(Prefix .. "Name", {Text = "New profile name", Placeholder = "Profile name", Finished = false})
    List.SkipProfile, Name.SkipProfile = true, true
    local function Refresh(Selected)
        local Success, Names = Store:List()
        if not Success then return false, Names end
        if Library.Unloaded then return false, "Menu closed." end
        List:SetValues(Names)
        List:SetValue(table.find(Names, Selected or List.Value) and (Selected or List.Value) or Names[1])
        return true, tostring(#Names) .. " cloud profiles."
    end
    local function Run(Action)
        if Busy or Library.Unloaded then return end
        Busy = true
        Status:SetText("Working...")
        task.spawn(function()
            local Called, Success, Result = pcall(Action)
            Busy = false
            if Library.Unloaded then return end
            local Message = tostring((Called and Result or Success) or "Cloud request failed.")
            Status:SetText(Message)
            if not Called or not Success then
                Library:Notify({Title = "Cloud profiles", Description = Message, Time = 5})
            end
        end)
    end
    local function Save(ProfileName, Overwrite)
        local Success, Result = Store:Save(ProfileName, Library:ExportProfile(), Overwrite)
        if not Success then return false, Result end
        local Listed, Error = Refresh(ProfileName)
        return true, "Saved " .. ProfileName .. (Listed and "." or (". Refresh failed: " .. tostring(Error)))
    end
    local function Confirm(Text, Action)
        if Busy or Library.Unloaded then return end
        Library:Notify({Title = "Cloud profiles", Description = Text, Time = 15, Actions = {
            {Text = "Confirm", Callback = function() Run(Action) end}, {Text = "Cancel"},
        }})
    end
    Container:AddButton({Text = "Refresh", Func = function() Run(function() return Refresh() end) end})
    Container:AddButton({Text = "Save new", Func = function()
        local ProfileName = Name:Commit()
        Run(function() return Save(ProfileName, false) end)
    end})
    Container:AddButton({Text = "Load", Func = function()
        local ProfileName = List.Value
        Run(function()
            local Success, Json = Store:Load(ProfileName)
            if not Success then return false, Json end
            if Library.Unloaded then return false, "Menu closed." end
            local Imported, Error = Library:ImportProfile(Json)
            return Imported, Imported and ("Loaded " .. ProfileName .. ".") or Error
        end)
    end})
    Container:AddButton({Text = "Replace selected", Func = function()
        local ProfileName = List.Value
        if not ProfileName then return end
        Confirm("Replace cloud profile " .. ProfileName .. " with your current settings?", function() return Save(ProfileName, true) end)
    end})
    Container:AddButton({Text = "Delete selected", Func = function()
        local ProfileName = List.Value
        if not ProfileName then return end
        Confirm("Delete cloud profile " .. ProfileName .. "?", function()
            local Success, Result = Store:Delete(ProfileName)
            if not Success then return false, Result end
            local Listed, Error = Refresh()
            return true, "Deleted " .. ProfileName .. (Listed and "." or (". Refresh failed: " .. tostring(Error)))
        end)
    end})
end
...
function Library:CreateWindow(WindowInfo)
    ...
    function Window:AddAccountStatusTab()
        ...
        local KeyLabel = Access:AddLabel("LuaProt: Loading")
        local ExpiryLabel = Access:AddLabel("Time Remaining: Unavailable")
        local Lifetime = false
        local LifetimeGradient = New("UIGradient", {
            Enabled = false,
            Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, Color3.fromRGB(170, 112, 15)),
                ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255, 210, 80)),
                ColorSequenceKeypoint.new(1, Color3.fromRGB(190, 132, 25)),
            }),
            Parent = ExpiryLabel.TextLabel,
        })
        Library.Registry[ExpiryLabel.TextLabel].TextColor3 = function()
            return Lifetime and Color3.new(1, 1, 1) or Library.Scheme.FontColor
        end
        local Account = {
            Tab = AccountTab,
            PreviousTab = nil,
        ...
        function Account:Refresh()
            ...
            local Remaining = tonumber(LP_TIMELEFT)
            if Remaining and (Remaining ~= Remaining or math.abs(Remaining) == math.huge) then Remaining = nil end
            local HasSession = LP_SESSION_ID ~= nil and tostring(LP_SESSION_ID) ~= ""
                and tostring(LP_SESSION_ID) ~= "unknown"
            local Hwid = type(LP_FINGERPRINT) == "string" and LP_FINGERPRINT ~= ""
                and LP_FINGERPRINT ~= "unknown" and LP_FINGERPRINT or DetectHwid()
            local Digits = Hwid:gsub("%D", ""):sub(1, 2)
            HwidLabel:SetText("HWID: " .. (Digits ~= "" and Digits or "--") .. "***")
            KeyLabel:SetText("LuaProt: " .. (LPH_OBFUSCATED and "Protected build"
                or (HasSession or Remaining ~= nil) and "Runtime data available" or "Development build"))
            ...
            ExpiryLabel:SetText(Lifetime and "Lifetime" or ("Time Remaining: " .. Expiry))
            LifetimeGradient.Enabled = Lifetime
            ExpiryLabel.TextLabel.TextColor3 = Lifetime and Color3.new(1, 1, 1) or Library.Scheme.FontColor
        end
    ...
    function Window:AddSettingsTab(Info)
        ...
        if Library.CloudProfiles then
            Library:AddCloudProfileControls(ProfilesBox:AddTab("Cloud"), Prefix .. "Cloud")
        end
...
pcall(applyAutoloadTheme, library, route)
library:SetWatermarkVisibility(false)

local active = true
library:OnUnload(function()
    active = false
    if environment.hitechyloader == library then environment.hitechyloader = nil end
end)
...
local font = scheme.Font or Font.fromEnum(Enum.Font.Gotham)
local scriptId = route.url:match("/(%d+)$")
local keyFolder = "hitechy/loader/keys"
local keyPath = scriptId and (keyFolder .. "/" .. scriptId .. ".txt")
...
local function rounded(object, radius)
    make("UICorner", {CornerRadius = UDim.new(0, radius), Parent = object})
end

local function label(parent, text, position, size, textSize, color, bold)
    return make("TextLabel", {
        BackgroundTransparency = 1, FontFace = bold and Font.fromEnum(Enum.Font.GothamBold) or font,
        Position = position, Size = size, Text = text, TextColor3 = color,
        TextSize = textSize, TextWrapped = true, TextXAlignment = Enum.TextXAlignment.Left,
        Parent = parent,
    })
end
...
local function maskHwid()
    local hwid
    for _, detector in ipairs({gethwid or false, get_hwid or false}) do
        if type(detector) == "function" then
            local ok, value = pcall(detector)
            if ok and value and tostring(value) ~= "" then hwid = tostring(value); break end
        end
    end
    if not hwid then
        local ok, value = pcall(analytics.GetClientId, analytics)
        if ok and value and tostring(value) ~= "" then hwid = tostring(value) end
    end
    return hwid and ("•••• •••• " .. hwid:sub(-4)) or "unavailable"
end

local function readRememberedKey()
    if not (keyPath and isfile and readfile) then return nil end
    local found, exists = pcall(isfile, keyPath)
    if not found or not exists then return nil end
    local ok, value = pcall(readfile, keyPath)
...
local function forgetKey()
    if keyPath and isfile then
        pcall(function()
            if not isfile(keyPath) then return end
            if delfile then delfile(keyPath)
            elseif writefile then writefile(keyPath, "") end
        end)
    end
end

local function rememberKey(key)
    if not (keyPath and writefile and isfolder and makefolder) then return end
    pcall(function()
        if not isfolder("hitechy") then makefolder("hitechy") end
        if not isfolder("hitechy/loader") then makefolder("hitechy/loader") end
...
local function validateKey(key)
    if not scriptId then return false, "This game has no LuaProt script ID." end
    local ok, result = pcall(function()
        local source = game:HttpGet("https://sdk.luaprot.net/", true)
        local sdk = assert(loadstring(source, "LuaProt key check"))()
        assert(type(sdk) == "table" and type(sdk.checkKey) == "function", "LuaProt key check is unavailable")
        sdk.scriptId = scriptId
        return sdk:checkKey(key)
    end)
    if not ok then return false, "Could not reach LuaProt. Please try again.", false, false end
    if type(result) ~= "table" then return false, "LuaProt returned an invalid response.", false, false end
    if result.status == "VALID" then return true end
    local status = tostring(result.status or ""):upper()
    local message = tostring(result.message or "LuaProt did not accept this key.")
    return false, message, status:find("EXPIRE", 1, true) ~= nil or message:lower():find("expir", 1, true) ~= nil, true
end


local panel = make("CanvasGroup", {
    Name = "HubKeyMenu", AnchorPoint = Vector2.new(0.5, 0.5),
    BackgroundColor3 = scheme.BackgroundColor, Position = UDim2.fromScale(0.5, 0.5),
    Size = UDim2.fromOffset(700, 320), Parent = library.ScreenGui,
})
rounded(panel, 16)
make("UIStroke", {Color = scheme.OutlineColor, Thickness = 1.5, Parent = panel})
local panelScale = make("UIScale", {Scale = 1, Parent = panel})
local function resizePanel()
    local camera = workspace.CurrentCamera
    if not camera then return end
    local viewport = camera.ViewportSize
    panelScale.Scale = math.min(1, (viewport.X - 24) / 700, (viewport.Y - 24) / 320)
end
resizePanel()
local camera = workspace.CurrentCamera
if camera then
    local connection = camera:GetPropertyChangedSignal("ViewportSize"):Connect(resizePanel)
    library:OnUnload(function() connection:Disconnect() end)
end

label(panel, "hitechy", UDim2.fromOffset(32, 19), UDim2.fromOffset(220, 34), 25, scheme.FontColor, true)
label(panel, route.name, UDim2.fromOffset(34, 52), UDim2.fromOffset(285, 23), 13, scheme.AccentColor)
make("Frame", {BackgroundColor3 = scheme.OutlineColor, BorderSizePixel = 0,
    Position = UDim2.fromOffset(349, 82), Size = UDim2.fromOffset(1, 205), Parent = panel})

local close = make("TextButton", {
    Name = "Close", BackgroundColor3 = scheme.MainColor, FontFace = font,
    Position = UDim2.fromOffset(653, 18), Size = UDim2.fromOffset(29, 29),
    Text = "×", TextColor3 = scheme.FontColor, TextSize = 22, Parent = panel,
})
rounded(close, 7)
close.MouseButton1Click:Connect(function() if active then library:Unload() end end)

local avatar = make("ImageLabel", {
    Name = "PlayerAvatar", BackgroundColor3 = scheme.MainColor,
    Position = UDim2.fromOffset(34, 112), Size = UDim2.fromOffset(78, 78),
    Image = "rbxthumb://type=AvatarHeadShot&id=" .. tostring(player.UserId) .. "&w=150&h=150",
    Parent = panel,
})
rounded(avatar, 12)
make("UIStroke", {Color = scheme.OutlineColor, Parent = avatar})
label(panel, "WELCOME,", UDim2.fromOffset(131, 113), UDim2.fromOffset(190, 24), 13, scheme.AccentColor, true)
label(panel, player.DisplayName or player.Name, UDim2.fromOffset(131, 138), UDim2.fromOffset(196, 36), 22, scheme.FontColor, true)
label(panel, "@" .. player.Name, UDim2.fromOffset(131, 171), UDim2.fromOffset(196, 22), 12, scheme.FontColor)
label(panel, "DEVICE ID", UDim2.fromOffset(34, 225), UDim2.fromOffset(170, 20), 12, scheme.AccentColor, true)
label(panel, maskHwid(), UDim2.fromOffset(34, 245), UDim2.fromOffset(285, 25), 15, scheme.FontColor)

label(panel, "Enter your LuaProt key", UDim2.fromOffset(380, 96), UDim2.fromOffset(280, 30), 20, scheme.FontColor, true)
label(panel, "A valid key will be remembered for next time.",
    UDim2.fromOffset(380, 129), UDim2.fromOffset(280, 40), 12, scheme.FontColor)
local keyInput = make("TextBox", {
    Name = "KeyInput", BackgroundColor3 = scheme.MainColor, ClearTextOnFocus = false,
    FontFace = font, PlaceholderText = "Paste your key here", PlaceholderColor3 = scheme.OutlineColor,
    Position = UDim2.fromOffset(380, 176), Size = UDim2.fromOffset(280, 39),
    Text = "", TextColor3 = scheme.FontColor, TextSize = 15,
    TextXAlignment = Enum.TextXAlignment.Left, Parent = panel,
})
rounded(keyInput, 8)
make("UIStroke", {Color = scheme.OutlineColor, Parent = keyInput})
make("UIPadding", {PaddingLeft = UDim.new(0, 12), PaddingRight = UDim.new(0, 12), Parent = keyInput})
local submit = make("TextButton", {
    Name = "SubmitKey", BackgroundColor3 = scheme.AccentColor,
    FontFace = Font.fromEnum(Enum.Font.GothamBold),
    Position = UDim2.fromOffset(380, 225), Size = UDim2.fromOffset(280, 40),
    Text = "Check key", TextColor3 = scheme.BackgroundColor, TextSize = 15,
    Parent = panel,
})
rounded(submit, 8)
local status = label(panel, "", UDim2.fromOffset(380, 271), UDim2.fromOffset(280, 35), 12, scheme.FontColor)


local busy = false
local function showStatus(message, success)
    status.Text = message
    status.TextColor3 = success and Color3.fromRGB(92, 210, 146) or scheme.FontColor
end

local function submitKey(key, remembered)
    if busy or not active then return end
    key = key:match("^%s*(.-)%s*$")
    if key == "" then showStatus("Enter a key to continue."); return end
    busy = true
    submit.Text = "Checking..."
    showStatus("Checking with LuaProt...")
    task.spawn(function()
        local valid, reason, expired, checked = validateKey(key)
        if not active then return end
        if not valid then
            if expired or (remembered and checked) then forgetKey() end
            busy = false
            submit.Text = "Check key"
            if remembered then keyInput.Text = "" end
            showStatus(expired and "Saved key expired. Enter a new key." or reason)
            return
        end

        environment.lp_key = key
        lp_key = key
        rememberKey(key)
        keyInput.Text = ""
        showStatus("Key found! It's time to load " .. route.name .. ".", true)
        submit.Text = "Loading..."
        local requestedPlace = game.PlaceId
        local downloaded, source = pcall(game.HttpGet, game, route.url, true)
        if not active then return end
        local compiled, execute, compileError
        if downloaded and type(source) == "string" and #source > 0 then
            compiled, execute, compileError = pcall(loadstring, source, route.name)
        end
        if not downloaded or not compiled or type(execute) ~= "function" then
            busy = false
            submit.Text = "Try again"
            keyInput.Text = key
            showStatus("Could not prepare the game script. Please try again.")
            warn("[hitechy loader] Could not prepare " .. route.name .. ": " .. tostring(compileError or source))
            return
        end
        task.wait(0.9)
        if not active then return end
        if game.PlaceId ~= requestedPlace or detect(game.PlaceId, game.GameId) ~= route then
            busy = false
            submit.Text = "Check key"
            showStatus("The place changed. Run the loader again.")
            return
        end
        library:Unload()
        task.spawn(function()
...
submit.MouseButton1Click:Connect(function() submitKey(keyInput.Text, false) end)
keyInput.FocusLost:Connect(function(enterPressed)
    if enterPressed then submitKey(keyInput.Text, false) end
end)

local rememberedKey = readRememberedKey()
if rememberedKey then submitKey(rememberedKey, true) end
